...
const ApaProductsPage = ({ onAddToCart }) => {
    ...
    useEffect(() => {
        API_PRODUCTS.getProducts((result, status, err) => {
            if (result !== null && status === 200) {
                // Filter only products containing "Apa"
                setProducts(result.filter(product => product.name.toLowerCase().includes("apa")));
            }
        });
    }, []);
    ...
    return (
        <Container className="mt-5">
            <h2 className="text-center mb-4 font-weight-bold">Apa Products</h2>
...
export default ApaProductsPage;
