...
const OtherProductsPage = ({ onAddToCart }) => {
    ...
    useEffect(() => {
        API_PRODUCTS.getProducts((result, status, err) => {
            if (result !== null && status === 200) {
                setProducts(result.filter(product =>
                    !product.name.toLowerCase().includes("pasta") &&
                    !product.name.toLowerCase().includes("periuta") &&
                    !product.name.toLowerCase().includes("apa")
                ));
            }
        });
    }, []);
    ...
    return (
        <Container className="mt-5">
            <h2 className="text-center mb-4 font-weight-bold">Alte Produse</h2>
