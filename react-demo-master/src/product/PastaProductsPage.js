...
const PastaProductsPage = ({ onAddToCart }) => {
    ...
    useEffect(() => {
        API_PRODUCTS.getProducts((result, status, err) => {
            if (result !== null && status === 200) {
                // Filter only "Pasta" products
                setProducts(result.filter(product => product.name.toLowerCase().includes("pasta")));
            }
        });
    }, []);
    ...
    return (
        <Container className="mt-5">
            <h2 className="text-center mb-4 font-weight-bold">Pasta</h2>
                    ...
                    <Col sm="6" md="4" lg="3" key={product.id} className="mb-4">
                        <Card className="shadow-sm border-0 rounded text-center p-3">
                            <div className="d-flex justify-content-center">
                                <img src={`/images/${product.name}.jpg`} alt={product.name} className="rounded" 
...
export default PastaProductsPage;
